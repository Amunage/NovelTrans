...
## 빠른 사용법
- 압축을 풀고 나온 NovelTrans.exe를 실행합니다.
- 웹소설 추출은 바로 사용할 수 있습니다.
- 환경 설정 > 모델 다운로드 페이지에서 Gemma4 모델을 다운받습니다.
- 환경 설정 > ENV 관리 페이지에서 다운로드 한 모델의 경로를 지정합니다. (예: models/gemma-4-E4B-it-Q4_K_M.gguf)
- 번역/용어집 메뉴에 진입하면 llama.cpp를 자동으로 다운로드 합니다.
- 환경 설정의 시스템 진단으로 누락된 파일이 있는지 확인합니다. (FAIL = 실행 불가 / WARN = 단순 주의)
...
- `69shuba.com`
  - 소설 홈 예시: `https://www.69shuba.com/book/90412.htm`
  - 목차 예시: `https://www.69shuba.com/book/90412/`
- `twkan.com`
  - 소설 홈 예시: `https://twkan.com/book/93323.html`
  - 목차 예시: `https://twkan.com/book/93323/index.html`
...
메인 메뉴의 `[7] 환경 설정`에서 주요 값을 바꿀 수 있습니다.
...
환경 설정의 업데이트 항목에서 자동업데이트를 지원합니다.

## 트러블슈팅

### `[ERROR] GGUF 모델이 없습니다. 설정을 확인해주세요.`

- 모롱 다운로드가 완료되지 않았거나 `LLAMA_MODEL_PATH`가 잘못된 경우입니다.
- `[7] 환경 설정 -> [2] 모델 다운로드`를 다시 실행해 보세요.
- 또는 `data/.env`의 `LLAMA_MODEL_PATH`가 실제 파일을 가리키는지 확인하세요.
- 경로 예시: `models/gemma-4-E4B-it-Q4_K_M.gguf`
...
- `source/작품명/*.txt` 같은 형식의 파일이 필요합니다.
...
- VRAM이 부족할 수 있습니다.
- 대략 GGUF 모델의 파일 용량 + 1GB 이상을 권장 VRAM으로 계산합니다.
- VRAM에 여유가 있는 모델을 선택하고 가급적이면 다른 프로세스를 종료해주세요.
